...
const calculate = (n1, operator, n2) => {
  ...
  for (let index = 0; index < secondNum; index++) {
    potencia=potencia*firstNum;};
  if (operator === 'add') return firstNum + secondNum;
  if (operator === 'substract') return firstNum - secondNum;
  if (operator === 'multiply') return firstNum * secondNum;
  if (operator === 'divide') return firstNum / secondNum;
  if (operator === 'porc') return (firstNum * (secondNum / 100));
  if (operator === 'raiz') return Math.sqrt(firstNum);
  if (operator === 'pot') return Math.pow(firstNum,secondNum);
...
const createResultString = (key, displayedNum, state) => {
  ...
  if (keyType === 'calculate') {
    return firstValue ?
      previousKeyType === 'calculate' ?
        calculate(displayedNum, operator, porcValue) :
        calculate(firstValue, operator, displayedNum) :
      displayedNum;
  }
};
...
const updateCalculatorState = (key, calculator, calculatedValue, displayedNum) => {
  ...
  if (keyType === 'calculate') {
    calculator.dataset.porcValue = firstValue && previousKeyType === 'calculate' ?
      porcValue :
      displayedNum;
  }
